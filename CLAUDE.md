...
This project is developed using both **Claude** and **Gemini (via Antigravity)**. This file (`CLAUDE.md`) is the instruction file for Claude. An equivalent `gemini.md` exists for Gemini. Both files must stay in sync — if you update project context, rules, or structure here, mirror the changes in `gemini.md` as well.
